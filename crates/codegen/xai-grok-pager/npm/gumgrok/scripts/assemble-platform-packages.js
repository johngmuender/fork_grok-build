...
//   1. Brotli-compresses the built binary into `../gumgrok-<platform>/bin/<bin>.br`
...
// (`@xai-official/gumgrok`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw gumgrok binary is
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `gumgrok-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] gumgrok-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'gumgrok',
            envVar: 'GROK_DARWIN_ARM64',
            defaultSource: path.join(xaiRoot, 'target', 'release', 'gumgrok'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'gumgrok',
            envVar: 'GROK_DARWIN_X64',
            defaultSource: path.join(xaiRoot, 'target', 'x86_64-apple-darwin', 'release', 'gumgrok'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'gumgrok',
            envVar: 'GROK_LINUX_X64',
            defaultSource: path.join(xaiRoot, 'target',
                'explorer_cross_x86_64-unknown-linux-gnu',
                'x86_64-unknown-linux-gnu', 'release', 'gumgrok'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'gumgrok',
            envVar: 'GROK_LINUX_ARM64',
            defaultSource: path.join(xaiRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'gumgrok'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'gumgrok.exe',
            envVar: 'GROK_WIN32_X64',
            defaultSource: path.join(xaiRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'gumgrok.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'gumgrok.exe',
            envVar: 'GROK_WIN32_ARM64',
            defaultSource: path.join(xaiRoot, 'target', 'aarch64-pc-windows-msvc', 'release', 'gumgrok.exe'),
        },
    ];
