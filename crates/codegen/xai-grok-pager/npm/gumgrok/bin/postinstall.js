#!/usr/bin/env node
// Runs once after npm install/update. Reads the gumgrok binary from the
// matching per-platform optional dependency (@xai-official/gumgrok-<platform>)
// and installs it to ~/.grok/bin/ using versioned filenames:
//
//   Unix:    gumgrok-<version>  +  gumgrok  (symlink)
//   Windows: gumgrok-<version>.exe  +  gumgrok.exe  (copy)
//
...
if (!SUPPORTED.has(key)) {
    console.error(`@xai-official/gumgrok: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@xai-official/gumgrok-${key}`;
    try {
...
if (!version) {
    console.error('@xai-official/gumgrok: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@xai-official/gumgrok: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@xai-official/gumgrok: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running gumgrok processes and try again.');
                return false;
            }
        }
    } else {
...
// Uses an exact prefix match + hyphen + digit to avoid gumgrok-* matching gumgrok-pager-*.
...
if (!platformDir) {
    console.error(`@xai-official/gumgrok: platform package @xai-official/gumgrok-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @xai-official/gumgrok');
    process.exit(0);
}

installBinary('gumgrok', platformDir, `gumgrok${EXE}`);
cleanupOldVersions('gumgrok');
// Legacy cleanup: prune binaries from prior `grok`-named installs.
...
// Persist the npm registry so `gumgrok update` and the trampoline use the same one.
...
const GROK_PATH = path.join(CANONICAL_DIR, `gumgrok${EXE}`);
if (process.env.GROK_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(os.homedir(), '.grok', 'completions');
        const bashPath = path.join(completionsDir, 'bash', 'gumgrok.bash');
        const zshPath = path.join(completionsDir, 'zsh', '_gumgrok');
    ...
    console.log('Tip: gumgrok completions bash > ~/.local/share/bash-completion/completions/gumgrok');
    console.log('     gumgrok completions zsh  > ~/.zsh/completions/_gumgrok');
}
