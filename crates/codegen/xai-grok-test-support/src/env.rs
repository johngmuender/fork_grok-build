...
fn local_grok_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("gumgrok{}", std::env::consts::EXE_SUFFIX))
}
...
fn ensure_local_grok_binary(binary: &Path) {
    ...
    let output = Command::new(&cargo)
        ...
        .args(["build", "-p", "xai-grok-pager-bin", "--bin", "gumgrok"])
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build gumgrok: {e}"));

    assert!(
        output.status.success(),
        "failed to build gumgrok for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "gumgrok build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve grok binary: `GROK_BINARY` env (CI) or a locally built `gumgrok` binary.
pub fn grok_binary() -> PathBuf {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_gumgrok") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
